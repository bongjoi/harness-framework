...
- `phase`: task 이름. 브랜치명(`feat-{phase}`)과 커밋 scope(`feat({phase})`)의 근거가 된다. 디렉토리명과 같게 두는 것을 권장하되, 디렉토리에 순번 prefix(`0-`)를 붙였다면 달라도 된다. 단, **phase 값은 phase 간 고유해야 한다** — 값이 겹치면 서로 다른 task가 같은 브랜치/커밋 scope를 공유해 충돌한다.
...
CLAUDE.md의 `## 명령어`에 정의된 빌드/테스트 커맨드를 실제 실행 가능한 형태로 적는다.
프로젝트 스택에 맞춰라 — Node가 아니면 npm을 쓰지 마라.

```bash
# 예시(Node): 실제로는 CLAUDE.md의 명령어를 사용할 것
...
3. 결과에 따라 `phases/{task-name}/index.json`의 해당 step을 업데이트한다:
   ...
   - AC를 만족시키지 못하면 → `"status": "error"`, `"error_message": "구체적 에러 내용"` (자체 재시도 루프를 돌리지 마라. execute.py가 자동으로 재시도한다)
...
python3 scripts/execute.py {task-name} --step N # step N 하나만 (상태 초기화 후) 재실행
python3 scripts/execute.py {task-name} --from N # step N부터 끝까지 (상태 초기화 후) 재실행
```

`--step`/`--from`은 해당 범위의 step status·메타데이터를 자동으로 초기화한 뒤 실행하므로, 디버깅 시 index.json을 손으로 고칠 필요가 없다. 부분 실행으로 끝나면 phase는 `completed`로 마킹되지 않는다.

execute.py가 자동으로 처리하는 것:

- 사전 검증 — 실행 전 `claude` CLI 존재, step 번호 중복, 실행 대상 `step{N}.md` 파일 존재, phase 값 고유성을 검사해 브랜치 생성 전에 빠르게 실패한다(fail-fast)
...
- **error 발생 시**: 코드를 점검한 뒤 `--step N`(해당 step만) 또는 `--from N`(이후 전체)으로 재실행한다. 상태 초기화는 자동이다. 인자 없이 재실행하면 `_check_blockers`가 error step에서 멈춘다.
- **blocked 발생 시**: `blocked_reason`에 적힌 사유(API 키 등)를 먼저 해결한 뒤 `--step N`으로 재실행한다.
